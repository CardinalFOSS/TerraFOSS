...
{
	public class VideoBeam : ModProjectile
	{
		// A helpful math constant for performing beam angling calculations.
		private const float PiBeamDivisor = MathHelper.Pi / VideoBeamHold.NumBeams;
		...
		{
			DisplayName.SetDefault("Video Beam");
		}
		...
		public override void AI()
		{
			...
			if (projectile.type != ModContent.ProjectileType<VideoBeam>() || !hostPrism.active || hostPrism.type != ModContent.ProjectileType<VideoBeamHold>())
			{
				projectile.Kill();
				return;
			}
			...
			float chargeRatio = MathHelper.Clamp(hostPrism.ai[0] / VideoBeamHold.MaxCharge, 0f, 1f);
			...
			projectile.friendly = hostPrism.ai[0] > VideoBeamHold.DamageStart;

			// This offset is used to make each individual beam orient differently based on its Beam ID.
			float beamIdOffset = BeamID - VideoBeamHold.NumBeams / 2f + 0.5f;
			float beamSpread;
			float spinRate;
			...
			float deviationAngle = (hostPrism.ai[0] + beamIdOffset * spinRate) / (spinRate * VideoBeamHold.NumBeams) * MathHelper.TwoPi;
		...
		private Color GetOuterBeamColor()
		{
			...
			float hue = (BeamID / VideoBeamHold.NumBeams) % BeamHueVariance + BeamColorHue;
